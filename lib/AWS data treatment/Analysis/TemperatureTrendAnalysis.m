...
for ii = 1:length(station_list)
        ...
        avg_temp{2} = AvgTable(data_JJA,'yearly','mean',50);
        ...
        for i =1:5
            ...
            if p_value(i) < p_thres
                h(i) = plot(years, p{i}.Coefficients.Estimate(1) + p{i}.Coefficients.Estimate(2).*years,'Color', color(i,:),'LineWidth',2.5);
                legtext{1} = sprintf('Year  (p value: %0.2f slope: %0.3f)',p_value(i), p{i}.Coefficients.Estimate(2));
            end
        end
       
        set(gca,'Position',get(gca,'Position')-[0 -0.14 0 0.25])
        legendflex(h, legtext, 'ref', gcf, ...
            'anchor', {'n','n'}, ...
            'buffer',[10 -10], ...
            'ncol',2, ...
            'fontsize',12);
        ...
        print(f,sprintf('%s/AirTemp_%s',OutputFolder,station),'-dtiff')
